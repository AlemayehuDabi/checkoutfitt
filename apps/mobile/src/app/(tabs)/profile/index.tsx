...
export default function ProfileScreen() {
  ...
  const userEmail = user?.email || "guest@checkoutfitt.com";
    ...
    <ScreenContainer scroll edges={["top", "left", "right"]}>
      ...
      <View className="flex-row items-center gap-lg">
        <View className="h-16 w-16 items-center justify-center rounded-full border-2 border-border bg-primary-100">
          <Text className="text-h2 font-bold text-primary-700">{initials}</Text>
        </View>
        <View className="flex-1">
          <Text className="text-h3 font-semibold text-text-primary">{userName}</Text>
          <Text className="mt-0.5 text-caption text-text-muted">{userEmail}</Text>
