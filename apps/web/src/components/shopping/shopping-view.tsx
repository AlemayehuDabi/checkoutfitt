...
export function ShoppingView() {
  ...
  /* The controls stay put while results appear beside them, so you can swap
     the photo and re-run without losing the verdict you're comparing against. */
  const panel = (
    <>
      ...
      <p className="mt-sm text-sm text-text-secondary">
        We&apos;ll check it against what you already own and tell you honestly
      ...
      <div className="mt-xl">
        <PhotoDropzone
          ...
          previewClassName="aspect-square w-full rounded-xl"
        />
      </div>

      <Divider label="or paste a link" className="my-xl" />
      ...
      <div className="mt-xl flex flex-col gap-md">
        <Button
          className="w-full"
          disabled={!canEvaluate || stage === "evaluating"}
          loading={stage === "evaluating"}
          onClick={evaluate}
          iconLeft={<ShoppingBag className="size-4" />}
        >
          {stage === "result" ? "Evaluate again" : "Evaluate this item"}
        </Button>
        {!canEvaluate && (
    ...
    <Workbench panel={panel}>
      {stage === "input" && (
        <WorkbenchIdle
          ...
          description="Drop in a product photo or paste a link, and the verdict will appear here alongside outfits it would unlock."
        />
      )}
      ...
      {stage === "evaluating" && (
        <div>
          ...
          <div className="grid gap-2xl md:grid-cols-[minmax(0,0.8fr)_minmax(0,1fr)]">
      ...
      {stage === "result" && result && (
        <WorkbenchResult>
          <div className="grid gap-2xl md:grid-cols-[minmax(0,0.8fr)_minmax(0,1fr)] md:items-start">
            {/* Product */}
            <div>
              {photo ? (
            ...
            {/* Verdict */}
            <div>
              <p className="text-eyebrow uppercase text-text-muted">Our verdict</p>
              <div className="mt-md">
            ...
            >
              {result.suggestedOutfits.map((outfit) => (
                <motion.li
                  key={outfit.name}
                  variants={listItem}
                  className="min-w-0 overflow-hidden rounded-xl border border-border bg-surface p-xl shadow-md transition-all duration-200 hover:-translate-y-[3px] hover:border-border-strong hover:shadow-lg"
                >
                  <p className="text-body-semibold text-text-primary">
                    {outfit.name}
                  </p>
